...
function fakeAI(vec: number[]) {
  return {
    run: vi.fn(async () => ({ shape: [1, vec.length], data: [vec] })),
  } as any;
}
function fakeVectorize() {
  ...
  return {
    ...
    query: vi.fn(async (_v: number[], opts: any) => ({
      ...
      matches: store
        .slice(0, opts.topK)
        .map((s) => ({ id: s.id, score: 0.9, metadata: s.metadata })),
    })),
  } as any;
}
...
describe("VectorIndex", () => {
  it("embeds text and passes data[0] to query", async () => {
    ...
    expect(ai.run).toHaveBeenCalledWith("@cf/qwen/qwen3-embedding-0.6b", {
      text: ["hello"],
    });
    expect(vz.query).toHaveBeenCalledWith(
      [0.1, 0.2, 0.3],
  ...
  it("upserts a memory with id + snippet metadata", async () => {
    ...
    expect(vz._store[0].metadata).toMatchObject({
      snippet: "buy milk",
      kind: "note",
    });
  });
